    ...
    User.findOne({ _id: ObjectID(req.decoded.userId) }, (err, user) => {
      if (err) {
        ...
        if (user.username != req.body.username) {
          ...
          User.findOne({ _id: ObjectID(req.decoded.userId) })
            .remove()
            .exec()
                    ...
                    async.each(
                      ...
                      function allDone(err) {
                        if (err) {
                          ...
                          Anime.update(
                            { user: { $in: memberNames } },
                            { $pull: { recommenders: { name: req.body.username } } },
                            { multi: true },
                            err => {
                              if (err) {
                                res.json({ success: false, message: err })
    ...
    User.findOneAndUpdate(
      { _id: ObjectID(req.decoded.userId) },
      {
        $set: {
          ...
          autoTimelineAdd: req.body.autoTimelineAdd,
          fireworks: req.body.fireworks,
        },
      },
      err => {
        if (err) {
