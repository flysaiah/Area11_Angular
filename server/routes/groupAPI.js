...
module.exports = router => {
    ...
    Group.findOne({ name: req.body.groupName }, (err, group) => {
      if (err) {
        ...
        if (found) {
          ...
          Group.findOneAndUpdate({ name: req.body.groupName }, { $set: { members: newMembers } }, err => {
            if (err) {
              res.json({ success: false, message: err })
            } else {
      ...
      if (err) {
        ...
        if (count == 1) {
          Group.findOne({ name: req.body.groupName })
            .remove()
            .exec()
          TopTens.find({ group: req.body.groupName })
            .remove()
            .exec()
            ...
            if (err) {
              ...
              User.findOneAndUpdate({ _id: ObjectID(req.decoded.userId) }, { $set: { group: '' } }, (err, user) => {
                if (err) {
                  ...
                  Anime.update(
                    { user: user.username, ownerIsRecommender: true },
                    { $set: { recommenders: [{ name: user.username }] } },
                    { multi: true },
                    err => {
                      if (err) {
                        res.json({ success: false, message: err })
                                  ...
                                  async.each(
                                    ...
                                    function(toptensObj, done2) {
                                      if (!toptensObj.entries) {
                                        ...
                                        TopTens.update(
                                          { category: toptensObj.category, user: toptensObj.user },
                                          { entries: newEntries },
                                          done2,
                                        )
                                      }
                                    ...
                                    function allDone2(err) {
                                      if (err) {
                                        ...
                                        TopTens.find({ user: user.username })
                                          .remove()
                                          .exec()
                                        // Update recommendations
                                        Anime.update(
            ...
            if (pUser.group) {
              ...
              Group.findOneAndUpdate({ name: req.body.groupName }, { $set: { members: newMembers } }, (err, group) => {
                if (err) {
                  ...
                  User.findOneAndUpdate({ _id: ObjectID(req.body.pendingUser) }, { $set: { group: req.body.groupName } }, err => {
                    if (err) {
                      res.json({ success: false, message: err })
                    } else {
                        ...
                        function allDone(err) {
                          if (err) {
                            ...
                            Anime.find({ user: { $in: memberNames } }, (err, groupAnimeList) => {
                              ...
                              async.eachSeries(
                                groupAnimeList,
                                function updateNewMemberRecommendations(groupMembAnime, done3) {
                                  if (!groupMembAnime.malID || malIDs.has(groupMembAnime.malID) || !groupMembAnime.recommenders) {
                                    done3()
                                  } else {
                                    malIDs.add(groupMembAnime.malID)
                                    Anime.update(
                                      { malID: groupMembAnime.malID, user: newMemberName },
                                      { $push: { recommenders: { $each: groupMembAnime.recommenders } } },
                                      done3,
                                    )
                                  }
                                },
                                  ...
                                  if (err) {
                                    ...
                                    Anime.find({ user: newMemberName }, (err, animeList) => {
                                      if (err) {
                                        ...
                                        async.each(
                                          animeList,
                                          function updateRecommendations(membAnime, done2) {
                                            if (!membAnime.ownerIsRecommender || !membAnime.malID) {
                                              done2()
                                            } else {
                                              Anime.update(
                                                { malID: membAnime.malID, user: { $in: memberNames } },
                                                { $push: { recommenders: { name: newMemberName } } },
                                                { multi: true },
                                                done2,
                                              )
                                            }
  ...
  router.post('/saveGroupChanges', (req, res) => {
    Group.findOneAndUpdate({ name: req.body.groupName }, { $set: { name: req.body.groupChangesModel.name } }, err => {
      if (err) {
        res.json({ success: false, message: err })
        ...
        async.each(
          ...
          function allDone(err) {
            if (err) {
              ...
              Anime.update(
                { user: { $in: memberNames }, ownerIsRecommender: false },
                { $set: { recommenders: [] } },
                { multi: true },
                err => {
                  if (err) {
                    ...
                    async.each(
                      memberNames,
                      function updateMemberRecommendations(memberName, done) {
                        Anime.update(
                          { user: memberName, ownerIsRecommender: true },
                          { $set: { recommenders: [{ name: memberName }] } },
                          { multi: true },
                          done,
                        )
                      },
          ...
          Anime.find({ user: req.body.fromUser }, (err, fromUserList) => {
            if (err) {
              ...
              if (!fromUserList) {
                ...
                async.each(
                  fromUserList,
                  function updateAnime(anime, done) {
                    if (anime['malID']) {
