...
module.exports = router => {
  ...
  router.post('/saveChanges', (req, res) => {
    ...
    TopTens.findOneAndUpdate({ category: newTT.category, group: newTT.group, user: newTT.user }, newTT, { upsert: true }, err => {
      if (err) {
        res.json({ success: false, message: err })
      } else {
  ...
  router.post('/deleteCategory', (req, res) => {
    TopTens.find({ category: req.body.category, group: req.body.groupName })
      .remove()
      .exec()
