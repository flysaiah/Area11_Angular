...
export class TopTensComponent implements OnInit {
  ...
  moveUp(category: string, catIndex: number, entryIndex: number) {
    ...
    const entries = this.topTensMap.get(category).get(this.categoryLogistics[catIndex]['member'])['entries']
  ...
  addNewEntry(category: string, catIndex: number, entryIndex: number) {
    ...
    const newViewerPrefs = []
    for (const gMemb of this.currentGroupMembers) {
      if (gMemb.username !== this.currentUser) {
        newViewerPrefs.push({ member: gMemb.username, shouldHide: false })
      }
    }
    const newEntry = { name: '', viewerPrefs: newViewerPrefs }
  ...
  private organizeTopTens() {
    ...
    for (const category of this.allCategories) {
      ...
      for (const member of this.currentGroupMembers) {
  ...
  isHidden(entry) {
    ...
    for (const memb of entry['viewerPrefs']) {
            ...
            if (resOne['user']['group']) {
              this.groupService.getGroupInfo(resOne['user']['group']).subscribe(resTwo => {
                if (resTwo['success']) {
                  ...
                  } else if (resTwo['message'] === 'Invalid group membership') {
