...
export class SettingsComponent implements OnInit {
  ...
  upload() {
    ...
    for (let i = 0; i < this.avatarUpload.length; i++) {
    ...
    this.authService.getProfile().subscribe(res => {
      if (res['success']) {
        ...
        this.userService.getUserInfo().subscribe(resOne => {
          if (resOne['success']) {
            this.model['bestgirl'] = resOne['user']['bestgirl']
            if (resOne['user']['autoTimelineAdd']) {
              this.model['autoTimelineAdd'] = resOne['user']['autoTimelineAdd']
            }
            if (resOne['user']['fireworks']) {
              this.model['fireworks'] = resOne['user']['fireworks']
            }
...
export class DeleteAccountDialogComponent {
  constructor(public dialogRef: MatDialogRef<DeleteAccountDialogComponent>, @Inject(MAT_DIALOG_DATA) public data: any) {}
  onNoClick(): void {
